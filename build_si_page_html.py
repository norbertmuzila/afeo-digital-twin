...
# 1. Add Sidebar Link
nav_link = """      <div class="nav-group">
        <div class="nav-group-label">Overview</div>
        <div class="nav-link active" data-page="dashboard"><span class="ni">🌍</span>Global Dashboard</div>
...
if '<div class="nav-group-label">Overview</div>' in html:
    start_idx = html.find('<div class="nav-group">\n        <div class="nav-group-label">Overview</div>')
    end_idx = html.find('</div>', start_idx) + 6
    html = html[:start_idx] + nav_link + html[end_idx:]
        ...
        <div class="si-content-area" style="flex:1; display:flex; gap:20px; min-height:0;">
          ...
          <div class="si-data-panel" style="width:450px; background:#fff; border:1px solid var(--border); border-radius:8px; display:flex; flex-direction:column; overflow-y:auto;">
            ...
            <div class="si-tab-content active" id="siTab-overview" style="padding:20px;">
              <div class="si-grid" id="siMetricsGrid" style="display:flex; flex-direction:column; gap:16px;">
                <!-- Cards injected here -->
              </div>
            </div>
            
            <div class="si-tab-content" id="siTab-evidence" style="padding:20px; display:none;">
              <div class="si-compare-list" id="siCompareList" style="display:flex; flex-direction:column; gap:20px;">
                <!-- Evidence/Compare injected here -->
              </div>
            </div>
            
            <div class="si-tab-content" id="siTab-regulatory" style="padding:20px; display:none;">
              <div class="si-regulatory-list" id="siRegList" style="display:flex; flex-direction:column; gap:16px;">
                <!-- Regulatory injected here -->
              </div>
            </div>
...
# Find closing of reports page to insert the new page before settings
if '<div class="page" id="pg-settings">' in html:
    html = html.replace('<div class="page" id="pg-settings">', new_page_html + '\n    <div class="page" id="pg-settings">')

with codecs.open("index.html", "w", encoding="utf-8") as f:
    f.write(html)

print("Dedicated Site Intelligence page added to index.html")
